...
export default withAuth(Sidebar,"all")   
function Sidebar (){
        ...
        <div className="min-h-screen flex flex-row bg-gray-100">
            <div className="flex flex-col w-56 bg-white rounded-r-3xl overflow-hidden">
                <div className="flex items-center justify-center h-24 shadow-md">
                    <h1 className="text-2xl uppercase text-indigo-500">POse Palace</h1>
                </div>
                    ...
                    <ul className="flex flex-col py-4">
                        {
                            links.map(({ href, label,icon }) => {
                                ...
                                return (
                                    <li key={`${href}${label}`}>
                                        <Link href={href} passHref legacyBehavior className='
                                    flex flex-row items-center h-12 transform hover:translate-x-2 transition-transform ease-in duration-200 text-gray-500 hover:text-gray-800'>
                                            <>
                                                <span className="inline-flex items-center justify-center h-12 w-12 text-lg text-gray-400"><current.icon /></span>
                                                <a href={href} className="text-sm font-medium">{label}</a>
                                            </>
                                        </Link>
                                    </li>
                                )
